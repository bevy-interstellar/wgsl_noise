...
fn noise_perlin_vec4f(p: vec4<f32>) -> f32 {
    ...
    gx00 -= sw00 * (step(vec4<f32>(0.0), gx00) - 0.5);
    gy00 -= sw00 * (step(vec4<f32>(0.0), gy00) - 0.5);
    ...
    gx01 -= sw01 * (step(vec4<f32>(0.0), gx01) - 0.5);
    gy01 -= sw01 * (step(vec4<f32>(0.0), gy01) - 0.5);
    ...
    gx10 -= sw10 * (step(vec4<f32>(0.0), gx10) - 0.5);
    gy10 -= sw10 * (step(vec4<f32>(0.0), gy10) - 0.5);
    ...
    gx11 -= sw11 * (step(vec4<f32>(0.0), gx11) - 0.5);
    gy11 -= sw11 * (step(vec4<f32>(0.0), gy11) - 0.5);
    ...
    let norm00 = inverseSqrt(vec4<f32>(
        dot(g0000, g0000),
        dot(g0100, g0100),
        dot(g1000, g1000),
        dot(g1100, g1100)
    ...
    let norm01 = inverseSqrt(vec4<f32>(
        dot(g0001, g0001),
        dot(g0101, g0101),
        dot(g1001, g1001),
        dot(g1101, g1101)
    ...
    let norm10 = inverseSqrt(vec4<f32>(
        dot(g0010, g0010),
        dot(g0110, g0110),
        dot(g1010, g1010),
        dot(g1110, g1110)
    ...
    let norm11 = inverseSqrt(vec4<f32>(
        dot(g0011, g0011),
        dot(g0111, g0111),
        dot(g1011, g1011),
        dot(g1111, g1111)
